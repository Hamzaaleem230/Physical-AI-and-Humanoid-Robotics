...
function ChatWindow({ isOpen, onClose, selectedText, setSelectedText }) {
    ...
    const sendQueryToBackend = async (userMessageText, context = null) => {
        ...
        try {
            ...
            setApiError(`Could not connect to backend. Please ensure the backend is running. (${error.message})`);
            setMessages((prevMessages) =>
                prevMessages.map((msg) =>
                    msg.id === botResponseId ? { ...msg, text: `Error: ${error.message}`, streaming: false } : msg
                )
            );
        } finally {
            setIsLoading(false);
    ...
    const handleExplainSelectedText = () => {
        if (selectedText.trim() && !isLoading) {
            const userMessage = { id: messages.length + 1, text: `Explain: "${selectedText}"`, sender: 'user' };
            setMessages((prevMessages) => [...prevMessages, userMessage]);
            setInput(''); 
            ...
            <div className={styles.messages}>
                ...
                {selectedText && (
                    <div className={styles.selectedTextPrompt}>
                        ...
                        <button onClick={() => setSelectedText('')} disabled={isLoading} className={styles.clearSelectedTextButton}>
                            Clear
                        </button>
                    </div>
                )}
